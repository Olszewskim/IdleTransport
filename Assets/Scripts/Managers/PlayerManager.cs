...
public class PlayerManager : Singleton<PlayerManager> {
    ...
    private void LoadSavedPlayer() {
        ...
        if (ZPlayerPrefs.HasKey(Constants.SAVE_PLAYER_KEY)) {
            var playerJSON = JsonConvert.DeserializeObject<PlayerJSON>(ZPlayerPrefs.GetString(Constants.SAVE_PLAYER_KEY), new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
